...
class PlantService(val view: PlantFragmentView) {
    fun tryGetGrowingPlant(){
        val signRetrofitInterface= Application.sRetrofit.create(PlantRetrofitInterface::class.java)
        signRetrofitInterface.getGrowingPlant().enqueue(object : Callback<GetGrowingPlantResponse>{
            override fun onResponse(call: Call<GetGrowingPlantResponse>, response: Response<GetGrowingPlantResponse>) {
                view.onGetGrowingPlantSuccess(response.body() as GetGrowingPlantResponse)
            }

            override fun onFailure(call: Call<GetGrowingPlantResponse>, t: Throwable) {
                view.onGetGrowingPlantFailure(t.message?:"통신 오류")
            }
